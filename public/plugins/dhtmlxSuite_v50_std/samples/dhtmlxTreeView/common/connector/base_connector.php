...
class EventInterface
{
    ...
    public $rules = []; //!< array of sorting rules
...
class Connector
{
    ...
    protected $options = []; //!< hash of OptionsConnector
    ...
    */
    public function __construct($db, $type = false, $item_type = false, $data_type = false, $render_type = false)
    {
        ...
        if (class_exists($type.'DBDataWrapper', false)) {
            $type .= 'DBDataWrapper';
        }
        if (!$item_type) {
            $item_type = 'DataItem';
        ...
        if ($this->live_update !== false && $this->updating !== false) {
            ...
            if ($this->editing) {
                ...
                if ($this->model && method_exists($this->model, 'get')) {
                    ...
                    $result = new ArrayQueryWrapper(call_user_func([$this->model, 'get'], $this->request));
    ...
    */
    protected function parse_request()
    {
        ...
        if (isset($_GET[self::$sort_var])) {
            foreach ($_GET[self::$sort_var] as $k => $v) {
                $k = $this->safe_field_name($k);
                $this->request->set_sort($this->resolve_parameter($k), $v);
            }
        }

        if (isset($_GET[self::$filter_var])) {
            foreach ($_GET[self::$filter_var] as $k => $v) {
                $k = $this->safe_field_name($k);
                if ($v !== '') {
    ...
    */
    ...
    {
        $action = new DataAction('deleted', $id, []);
        $request = new DataRequestConfig();
        $request->set_source($this->request->get_source());
    ...
    */
    public function enable_live_update($table, $url = false)
    {
        ...
        $this->event->attach('beforeOutput', [$this->live_update, 'version_output']);
        $this->event->attach('beforeFiltering', [$this->live_update, 'get_updates']);
        $this->event->attach('beforeProcessing', [$this->live_update, 'check_collision']);
        $this->event->attach('afterProcessing', [$this->live_update, 'log_operations']);
    }
    ...
    {
        $this->filters[] = ['name' => $name, 'value' => $value, 'operation' => $operation];
    }
