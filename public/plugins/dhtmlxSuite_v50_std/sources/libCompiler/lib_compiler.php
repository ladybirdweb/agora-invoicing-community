...
$skins = ['skyblue', 'web', 'terrace', 'material'];
if (!file_exists('../../skins')) {
    mkdir('../../skins');
}
if (!file_exists('../../codebase')) {
    mkdir('../../codebase');
}
...
foreach ($stat as $st => $output) {
    ...
    for ($q = 0; $q < count($files); $q++) {
        if (file_exists('../'.$files[$q]) && !is_dir('../'.$files[$q])) {
            $content .= file_get_contents('../'.$files[$q])."\n";
        }
    }
...
// js copy only
$files = array_values(array_map('trim', array_diff(explode("\n", file_get_contents('conf/stat_copy')), [''])));
...
for ($q = 0; $q < count($skins); $q++) {
    ...
    $s1 = [',M12', ',M21', ',M22', ',sizingMethod'];
    $s2 = [', M12', ', M21', ', M22', ', sizingMethod'];
