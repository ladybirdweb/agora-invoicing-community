...
class dhx_externalinput_clean
{
    ...
    public static function basic($string, $filterIn = ['Tidy', 'Dom', 'Striptags'], $filterOut = 'none')
    {
    ...
    {
        if (is_callable(['self', 'tidyUpModule'.$filter])) {
            return call_user_func(['self', 'tidyUpModule'.$filter], $string);
        }
...
class ConnectorSecurity
{
    ...
    public static function filter($value, $mode = false)
    {
        ...
        if ($mode == DHX_SECURITY_SAFEHTML) {
            if (self::$filterClass == null) {
                self::$filterClass = new dhx_externalinput_clean();
            }
    ...
    {
        LogMaster::log('[SECURITY] Possible CSRF attack detected', [
    ...
    public static function checkCSRF($edit)
    {
        if (self::$security_key) {
            ...
            if ($edit === true) {
                if (!isset($_POST[self::$security_var])) {
                    return self::CSRF_detected();
                }
                $master_key = $_SESSION[self::$security_var];
                $update_key = $_POST[self::$security_var];
            ...
            //data loading
            if (!array_key_exists(self::$security_var, $_SESSION)) {
                $_SESSION[self::$security_var] = md5(uniqid());
            }

            return $_SESSION[self::$security_var];
        }
