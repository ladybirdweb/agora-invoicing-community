...
use App\User;
use Carbon\Carbon;
use App\Plugins\Stripe\Controller\SettingsController;
...
class CronController extends BaseCronController
{
    ...
    public function autoRenewal()
    {
        ...
        try {
                ...
                if ($status == '1' && $subscription->rzp_subscription == '0') {
                    ...
                    if ($stripe_subscription['status'] == 'active') {
                        ...
                        if ($cost && emailSendingStatus()) {
                            ...
                            $stripeController->sendPaymentSuccessMailtoAdmin($currency, $cost,$user, $invoice->product_name);
                        }
                    }
                }
            }
            ...
            $stripeController->sendFailedPaymenttoAdmin($currency,$product_details->name,$cost,$e->getMessage(),$user)
            ...
            if ($subscription->autoRenew_status == 'Pending') {
                ...
                if ($subscriptionStatus['status'] == 'authenticated') {
                    ...
                    if ($invoice) {
                        ...
                        $stripeController->sendPaymentSuccessMailtoAdmin( $invoice->currency, $cost,$user, $product_name);
    ...
    public function activeSubs($subId, $subscription)
    {
        try {
            ...
            if ($subscriptionStatus['status'] == 'active') {
                ...
                $this->postRazorpayPayment($invoiceItem, $payment_method = 'Razorpay');
                $stripeController->sendPaymentSuccessMailtoAdmin($oldinvoice->currenc, $cost,$user,$product_details->name);
